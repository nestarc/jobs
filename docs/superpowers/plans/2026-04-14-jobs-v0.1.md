> **HISTORICAL / SUPERSEDED — DO NOT RESUME.** This implementation plan is retained as history. Use the [current maintenance plan](../../2026-09-02-p0-p4-maintenance-work-plan.md) for new work; old unchecked boxes are not a backlog.
...
**Architecture:** Two deliberately separate paths in v0.1. The InMemory path exercises the full fairness engine (`Scheduler` + `FairWorker`) and is the reference implementation of tenant-fair scheduling. The BullMQ path uses standard BullMQ `Worker` (autorun:true) for FIFO delivery, skipping the scheduler entirely — this is the **A안 tradeoff** we accepted after self-review showed `Worker.getNextJob()` cannot pick arbitrary jobs the way Pro Groups does. v0.2 closes the gap with Redis Lua scripts. Both paths share ALS context propagation (`__nestarcCtx` reserved key), `@JobHandler` decorator registration, and the outbox bridge. Module API exposes the tradeoff explicitly: `JobsModule.forInMemory(...)` vs `JobsModule.forBullMQ(...)`.
...
### A안 Tradeoff (explicit)
- ✅ **InMemory path**: full tenant fairness (scheduler + FairWorker). Used by `FakeJobsService` for consumer tests and anyone running in-process queues
- ✅ **BullMQ path**: ALS context propagation, `@JobHandler`, outbox bridge — all work. **Fairness does NOT**. Jobs are consumed FIFO by standard BullMQ Worker
- ⏭️ **v0.2**: Lua-script-based BullMQ consumer that respects the scheduler, achieving Pro Groups parity on Redis

### Included in v0.1
- ✅ `JobsBackend` interface with `InMemoryBackend` + `BullMQBackend`
- ✅ Shard-based weighted round-robin scheduler (InMemory path only)
- ✅ `FairWorker` (InMemory path only)
- ✅ `@JobHandler` decorator + `DiscoveryService` registration (both paths)
- ✅ ALS `contextExtractor` / `contextRunner` hooks (both paths)
- ✅ `JobsOutboxBridge` adapter (both paths)
- ✅ `FakeJobsService` for consumer tests

### Excluded from v0.1
- ❌ Tenant fairness on BullMQ path — v0.2 Lua scripts
...
## Task 8: BullMQ Backend (A안 — standard Worker, no scheduler)

**Files:**
- Create: `src/backend/bullmq-backend.ts`

Under A안, BullMQ backend does NOT implement the pull-based methods (`peekWaiting`/`moveToActive`/`ack`/`fail`) used by `FairWorker`. It exposes `enqueue` and a separate `startConsumer()` method that creates a standard BullMQ `Worker` (autorun: true). This is the honest, shippable path. Fairness with BullMQ comes in v0.2 via Lua scripts.
...
export class BullMQBackend implements JobsBackend {
  ...
  // Starts a standard BullMQ Worker per jobType. Jobs are delivered FIFO
  // by BullMQ. Scheduler is intentionally not consulted — see A안 tradeoff.
...
git commit -m "feat: BullMQ backend with standard Worker (A안 — fairness deferred to v0.2)"
```

Note:
- No CI test for BullMQ backend in v0.1 (requires Redis). Consumer smoke-tests against their Redis. v0.2 adds testcontainers-based integration test.
- `peekWaiting`/`moveToActive`/`ack`/`fail` throw explicit errors so accidental FairWorker use against BullMQ surfaces immediately rather than silently degrading.
- `getRawQueue(jobType)` provides escape hatch per spec §7.
...
## Task 12: JobsModule (forInMemory + forBullMQ)

**Files:**
- Create: `src/tenancy-defaults.ts`
- Create: `src/jobs.module.ts`

Under A안 the module API is **explicitly two-path**. Consumers choose at import:
- `JobsModule.forInMemory(...)` — full fairness, InMemory backend, FairWorker loop
- `JobsModule.forBullMQ(...)` — FIFO, standard BullMQ Worker. v0.2 will add `forBullMQFair(...)`

This makes the v0.1 tradeoff visible at call sites and prevents silent degradation.
      ...
      HandlerRegistry,
      {
        ...
        useFactory: (registry: HandlerRegistry) =>
          new JobsService({
            ...
            registry,
            // No schedulers on BullMQ path in v0.1. enqueue() will throw
            // QueueNotFound if jobType absent from this map, so we populate
            // with noop scheduler placeholders to allow enqueue lookup.
            schedulers: new Map(
              options.jobTypes.map((t) => [
                t,
                // Minimal stub — never pickNext'd on BullMQ path
...
git commit -m "feat: JobsModule.forInMemory + JobsModule.forBullMQ (A안 explicit split)"
```
